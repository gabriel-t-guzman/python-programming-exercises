...
def maxValorization(lst):
    vMax = {}
    map1 = {} # map1 {valor : empresa}
    map2 = {}# map2 {data : [valor1, valor2]}
    for i in lst: # fazer os dois mapas
        valor = round((i[5]/i[2] - 1)*100,2) # valorizacao
        map1.setdefault(valor, []).append(i[0]) # map1
        map2.setdefault(i[1], []).append(valor) #map2
    for i in map2:
        Vm = max(map2[i])
        E = map1[Vm]
        if len(E) > 1: E = tuple(E)
        else:
            E = E[0]
        t = (E, Vm)
        vMax[i] = t
