...
def telToName(tel, telList, nameList):
    # your code here
    if tel not in telList : return tel
    a = telList.index(tel)
    name = nameList[a]
    return name
...
def nameToTels(partName, telList, nameList):
    ...
    tels = []
    for i in nameList:
        if partName == i[0:a]: #con esto veo si coinciden el nombre con la parte escrita
            indices = nameList.index(i) # ahora quiero unos indices para tirar los numeros
            tels.append(telList[indices])
    return tels
